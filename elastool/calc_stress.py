...
def calc_stress(pos_optimized, cell_strain, method_stress_statistics, stress_set_dict, num_last_samples, up, cwd):
    ...
    if method_stress_statistics == 'dynamic':
        ...
        write('POSCAR.in', pos_strain, format='espresso-in', direct=True)
    else:
        #vasp.write_vasp('POSCAR', pos_strain, vasp5=True, sort=True, direct=True)
        write('POSCAR.in', pos_strain, format='espresso-in', direct=True,  kspacing=0.028, input_data={'ecutwfc' :80, 'ecutrho':600, 'calculation':'relax',  'prefix' : 'Aknew_','tprnfor' : True, 'tstress' : True, #'nstep':1,
                     'conv_thr' : 1.0e-9 ,  'mixing_beta' : 0.4, 'ion_dynamics':'bfgs', 'cell_dynamics':'bfgs',
                    },
            pseudopotentials={'O': 'O.wc-nl-rrkjus_psl.1.0.0.UPF',  'Si': 'Si.wc-nl-rrkjus_psl.1.0.0.UPF', 'Mg':'Mg.wc-spnl-rrkjus_psl.1.0.0.UPF'})
    ...
    if run_mode == 1 or run_mode == 3:
        stress = mean_stress('CONTCAR.out', num_last_samples, tag)
        total_energy = calculate_total_energy('CONTCAR.out')
        external_pressure = extract_external_pressure('CONTCAR.out')
    ...
    try:
        with open(outcar_file, 'r') as file:
            ...
            for line in reversed(lines):
                if 'Final enthalpy' in line.lower():

                    total_energy = float(line.split()[-2])*13.6057039763
                    break
...
def extract_external_pressure(outcar_filename):
    ...
    try:
        file=read(outcar_filename, format='espresso-out')
        external_pressure=np.mean(file.get_stress()[0:2])/-0.000625
