...
def get_stress(outcar_file, tag):
    ...
    #         num_record += 1
    file=read(outcar_file, format='espresso-out')
    df.append(np.array(file.get_stress())/-0.000625)
    df=DataFrame(df, columns=dirs)
...
def get_pressure(outcar_file):
    ...
    #         num_record += 1
    file=read(outcar_file, format='espresso-out')
    df.append(np.array(np.mean(file.get_stress()[0:2]))/-0.000625)
    df=DataFrame(df)
