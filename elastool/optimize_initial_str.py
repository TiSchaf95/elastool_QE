...
#    os.chdir('OPT')
    ...
    while attempt < max_retries:
        try:
            write('POSCAR.in', pos_conv, format='espresso-in', direct=True,  kspacing=0.028, input_data={'ecutwfc' :80, 'ecutrho':600, 'calculation':'vc-relax',  'prefix' : 'Aknew_','tprnfor' : True, 'tstress' : True,# 'nstep':1,
                         'conv_thr' : 1.0e-6 ,  'mixing_beta' : 0.7, 'ion_dynamics':'bfgs', 'cell_dynamics':'bfgs',
                        'press':150, 'press_conv_thr':0.05},
            pseudopotentials={'O': 'O.pbe-nl-rrkjus_psl.1.0.0.UPF',  'Si': 'Si.pbe-nl-rrkjus_psl.1.0.0.UPF', 'Mg':'Mg.pbe-spnl-rrkjus_psl.1.0.0.UPF'})
            kpoints_file_name = 'KPOINTS-static'
            pos_optimized = vasp_run(tag, kpoints_file_name, cwd)
            break  
            ...
            if attempt < max_retries and os.path.exists('CONTCAR') and os.path.getsize('CONTCAR') > 0:
                ...
                pos_optimized = read(os.path.join(opt_dir, 'CONTCAR'))
                pos_optimized = remove_spurious_distortion(pos_optimized)

                # Write the cleaned structure back to POSCAR
                write('POSCAR.in', pos_conv, format='espresso-in', direct=True, input_data={'ecutwfc' :80, 'ecutrho':600, 'calculation':'vc-relax',  'prefix' : 'Aknew_','tprnfor' : True, 'tstress' : True,# 'nstep':1,
                'conv_thr' : 1.0e-6 ,  'mixing_beta' : 0.7, 'ion_dynamics':'bfgs', 'cell_dynamics':'bfgs',
                'press':150, 'press_conv_thr':0.05},
            pseudopotentials={'O': 'O.pbe-nl-rrkjus_psl.1.0.0.UPF',  'Si': 'Si.pbe-nl-rrkjus_psl.1.0.0.UPF', 'Mg':'Mg.pbe-spnl-rrkjus_psl.1.0.0.UPF'})
